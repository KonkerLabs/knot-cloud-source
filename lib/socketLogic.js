    ...
    getDevice(socket, function(err, fromDevice){
      ...
      saveDataIfAuthorized(skynet.sendMessage, fromDevice, data.uuid, data, function(error){
        if(error){
          ...
          //Konker send data
          konkerApi.sendDataPromise("uqk8ilsm6872", "rT1HfMLQfV1r", data.data)
            .then(result => {
              console.log(result);
            });
        }
  ...
  socket.on('updateGatewayDeviceMapping', throttler.throttle(function(data, fn){
    knotGatewayManager.updateGatewayDeviceMapping(data,function(err, result) {
      if (err) {
        ...
        //konker
        //check if device exists in MONGO
        //if(!getDbCretentials(data.mappings[0]['thingUUID'])){  // TO DO  create this function -> getDbCretentials
          //if not, create it and save in MONGO
          //create a device in konker platform
          konkerApi.createDevicePromise(data.gatewayUUID, data.mappings[0]['thingUUID']).then(function (res) {
              ...
              (res) =>{
                ...
                //save device in MONGO
              }  
            ).catch(function (qqcoisa){
              console.log(qqcoisa);
            });
        //}
