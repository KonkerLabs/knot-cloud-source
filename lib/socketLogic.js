...
function socketLogic (socket, secure, skynet){
    ...
    getDevice(socket, function(err, fromDevice){
      ...
      saveDataIfAuthorized(skynet.sendMessage, fromDevice, data.uuid, data, function(error){
        if(error){
          ...
          //Konker send data
          konkerDataMongo.getDeviceCredentialsByUuidPromise(data.source)
          .then((res) => {
            if (res.length != 0) {
              let dataToKonker = Object.assign({}, data.data);
              delete(dataToKonker.uuid);
              delete(dataToKonker.token);

              konkerApi.sendDataPromise(res[0].apiKey, res[0].password, dataToKonker)
              .then(result => {
                console.log(result);
              });
  ...
  socket.on('updateGatewayDeviceMapping', throttler.throttle(function(data, fn){
    knotGatewayManager.updateGatewayDeviceMapping(data,function(err, result) {
      if (err) {
        console.error(err);
      } else if (knotGatewayManager.gateways.length > 0) {
        let userOwner = knotGatewayManager.gateways[0].gatewayDevice.owner;
        //konker
        konkerDataMongo.getDeviceCredentialsByUuidPromise(data.mappings[0]['thingUUID'])
          .then((res) => {
            if (res.length == 0) {
              konkerApi.createDevicePromise(userOwner, data.gatewayUUID, data.mappings[0]['thingUUID'])
                .then((res) => {
                  return konkerApi.getDeviceCredentialsPromise(
                    userOwner,
                    data.gatewayUUID,
                    res.data.result.guid)
                }).then((res) => {
                  console.log("konker device username:" + res.data.result.username + " password:" + res.data.result.password);
                  konkerDataMongo.saveDeviceCredentials(
                    data.mappings[0]['thingUUID'],
