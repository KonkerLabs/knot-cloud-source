    ...
    knotGatewayManager.updateGatewayDeviceMapping(data,function(err, result) {
      if (err) {
        ...
        konkerDataMongo.getDeviceCredentialsByUuidPromise(data.mappings[0]['thingUUID'])
          .then((res) => {
            if (res.length == 0) {
              konkerApi.createDevicePromise(data.gatewayUUID, data.mappings[0]['thingUUID'])
                .then((res) => {
                  return konkerApi.getDeviceCredentialsPromise(data.gatewayUUID, res.data.result.guid)
                }).then((res) => {
