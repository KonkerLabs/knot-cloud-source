  ...
  socket.on('updateGatewayDeviceMapping', throttler.throttle(function(data, fn){
    knotGatewayManager.updateGatewayDeviceMapping(data,function(err, result) {
      if (err) {
        ...
        knotGatewayManager.getThingGateway(data.gatewayUUID, function(gateway) {
          //konker
          konkerDataMongo.getDeviceCredentialsByUuidPromise(data.mappings[0]['thingUUID'])
            .then((res) => {
              if (res.length == 0) {
                konkerApi.createDevicePromise(gateway.owner, data.gatewayUUID, data.mappings[0]['thingUUID'])
                  .then((res) => {
                    ...
                    console.log("konker device username:" + res.username + " password:" + res.password);             
