...
function setupHttpRoutes(server, skynet){
  ...
  server.post('/devices', function (req, res) {
    ...
    knotDeviceManager.createUUID(function (uuid) {
      // config schema validation (will only run if there's a config array in req json)
      if (!_.isEmpty(req.body.config)) {
        knotPayloadValidation.validateConfig(req.body.config, function (errors) {
          if (!_.isEmpty(errors)) {
            meshbluEventEmitter.emit('register-error', errors);
        ...
        register(req.merged_params, function (error, device) {
          if (error) {
            ...
            konkerApi.createApplicationPromise(device.uuid);
            ...
            knotUser.getUserByUuid(req.merged_params['owner'], function (getUserErr, user) {
              if (getUserErr) {
                ...
                //TODO: create a Konker platform user with the same password from Knot
                //konkerApi.createApplicationPromise(
                //  user[0]['user']['uuid'],
                //  user[0]['user']['email'],
  ...
  // curl -X POST -d "user.email=email&user.password=passwordHash&type=user" http://localhost:3000/devices/user
  server.post('/devices/user', function (req, res) {
    skynet.sendActivity(getActivity('devices', req));
    req.merged_params.ipAddress = req.merged_params.ipAddress || getIP(req);
    if (!req.merged_params['user']['email'] || !req.merged_params['user']['password']) {
      return res.status(400).json({ message: 'Invalid e-mail or password' });
    } else {
      knotUser.getUserByEmail(req.merged_params['user']['email'], function (getUserErr, user) {
        if (getUserErr) {
          meshbluEventEmitter.emit('user-error', { request: req.merged_params, error: getUserErr });
          return res.status(500).json({ message: getUserErr.message });
        } else if (user) {
          ...
          return res.status(409).json({ message: 'User exists' });
            ...
            knotDeviceManager.createUUID(function (uuid) {
              register(req.merged_params, function (registerErr, device) {
                if (registerErr) {
                  meshbluEventEmitter.emit('register-error', { request: req.merged_params, error: registerErr.message });
                  return res.status(500).json({ message: registerErr.message });
                } else {
                  //KONKER API CALL TO CREATE USER
                  //konkerApi.createUserPromise(name, company, email, password)
                  konkerApi.createUserPromise(
                    device.user.email, 
                    undefined, 
                    device.user.email, 
                    ...
                    .then(result => {
                      console.log(result)
                      return konkerApi.getUserCredentialsPromise(
                        device.user.email,
                        device.user.password)
                    })
                    .then(result => {
                      konkerDataMongo.saveUserCredentials(
                        device.uuid,
                        device.user.email,
                        result.token)
                    })
    ...
    authorizeRequest(req, res, function (fromDevice) {
      skynet.sendActivity(getActivity('devices',req, fromDevice));
        ...
        if (!_.isEmpty(req.body.config)) {
          knotPayloadValidation.validateConfig(req.body.config, function (errors) {
            if (!_.isEmpty(errors)) {
              meshbluEventEmitter.emit('update-error', errors);
              return errorResponse(errors, res);
            } else {
              updateFromClient(fromDevice, _.defaults({ uuid: req.params.uuid }, req.merged_params), function (result) {
                ...
                if (result.error) {
                  meshbluEventEmitter.emit('update-error', {
                    request: requestLog,
  ...
  // curl -X DELETE http://localhost:3000/mydevices/ --header "meshblu_auth_uuid: {my uuid}" --header "meshblu_auth_token: {my token}"
  server.delete('/mydevices', function(req, res){
    var deviceCount;
    var output = [];
    var query = req.query || {};
    authorizeRequest(req, res, function(fromDevice){
      // get devices of the owner
      skynet.sendActivity(getActivity('mydevices', req, fromDevice));
      query.owner = fromDevice.uuid;
      getDevices(fromDevice, query, false, function(error, getResData){
        if(getResData.error){
          meshbluEventEmitter.emit('devices-error', {request: query, error: getResData.error.message, fromUuid: fromDevice.uuid});
          return errorResponse(getResData.error, res);
        }

        meshbluEventEmitter.emit('devices', {request: req.query, fromUuid: fromDevice.uuid});
        deviceCount = getResData.devices.length;
        // delete devices of the owner
        for(var i = deviceCount - 1; i >= 0; i--){
          skynet.sendActivity(getActivity('unregister', req, fromDevice));
          unregister(fromDevice, getResData.devices[i].uuid, req.merged_params.token, skynet.emitToClient, function(error, unregResData){
            if(error){
              meshbluEventEmitter.emit('unregister-error', {request: {uuid: unregResData.uuid}, fromUuid: fromDevice.uuid, error: error});
              return errorResponse(error, res);
            }

            meshbluEventEmitter.emit('unregister', {request: {uuid: unregResData.uuid}, fromUuid: fromDevice.uuid});
            output.push(unregResData.uuid);
            if(output.length === deviceCount)
              if(deviceCount === 1){
                res.json({"uuid":output[0]});
              }else{
  ...
  server.post('/messages', function(req, res, next){
    authorizeRequest(req, res, function(fromDevice){
      ...
      if (_.isEmpty(message.devices) && !message.target) {
  ...
  server.post('/data/:uuid', function(req, res){
    authorizeRequest(req, res, function(fromDevice){
      ...
      saveDataIfAuthorized(skynet.sendMessage, fromDevice, req.params.uuid, req.merged_params, function(error, data){
        ...
        if (knotParentConnection) {
          buildParentData(fromDevice, config, data, function (parentData) {
            knotParentConnection.socket.emit('data', parentData);
          });
        }
  ...
  server.get('/', function(req, res){
    res.redirect(301, 'http://knot.cesar.org.br/');
  });
  ...
  //TODO: check if that makes sense
  server.put('/addruletodevice/:uuid/:rule', function (req, res){
    authorizeRequest(req, res, function(fromDevice){
      skynet.sendActivity(getActivity('addrule',req, fromDevice));

      addRule(fromDevice, {uuid: req.params.uuid, rule: req.params.rule}, function(error, data){
        if(error){
          meshbluEventEmitter.emit('addrule-error', {request: {uuid: req.params.uuid}, error: error.message, fromUuid: fromDevice.uuid, fromIp: fromDevice.ipAddress});
          return res.json(400, error.message);
        }

        meshbluEventEmitter.emit('addrule', {request: {uuid: req.params.uuid}, fromUuid: fromDevice.uuid, fromIp: fromDevice.ipAddress});
        res.json(data);
      });
    });
  });
